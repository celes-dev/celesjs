...
import { AbiProvider, AuthorityProvider, AuthorityProviderArgs, BinaryAbi } from "./celesjs-api-interfaces";
import { base64ToBinary, convertLegacyPublicKeys } from "./celesjs-numeric";
import { GetAbiResult, GetBlockResult, GetCodeResult, GetInfoResult, GetRawCodeAndAbiResult, PushTransactionArgs } from "./celesjs-rpc-interfaces"; // tslint:disable-line
import RpcError from "./celesjs-rpcerror";
...
export default class JsonRpc implements AuthorityProvider, AbiProvider {
    ...
    public async get_question_block_number() {
        return await this.fetch("/v1/history/get_question_block_number", {});
    }

    public async get_block_random(block_num_or_id : string){
        return await this.fetch("/v1/history/get_block_random", {block_num_or_id});
    }

    public async verify_wood(block_number : number,account:string,wood:string){
        return await this.fetch("/v1/history/verify_wood", {block_number,account,wood});
    }
} // JsonRpc

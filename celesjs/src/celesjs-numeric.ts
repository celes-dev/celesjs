...
export function stringToPublicKey(s: string): Key {
    ...
    if (s.substr(0, 5) === "CELES") {
        const whole = base58ToBinary(publicKeyDataSize + 4, s.substr(5));
...
/** If a key is in the legacy format (`celes` prefix), then convert it to the new format (`PUB_K1_`).
